...
A sensor card displays a live reading from a Home Assistant sensor entity. It shows a large value with an optional unit and a label underneath — useful for temperatures, humidity, power usage, or text states such as fan speed.
...
3. Set a **Unit** — the unit label shown next to the value (for example, `°C`, `%`, `W`, or `kWh`). Leave this blank for text states such as `low`, `medium`, or `high`.
...
- Text values are displayed as-is. The precision setting only affects numeric values.
...
| `sensor.fan_level` |  | Fan | Text states such as `low`, `medium`, or `high` |
